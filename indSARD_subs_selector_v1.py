...
# -*- coding: utf-8 -*-
"""
Created on Fri May 10 10:07:16 2019

@author: emilhenningsen
"""


##### Industry SARD identifier 
...
# Industry SARD algorithm dynamic
# setup
# list for looping
...
grouped_median_errors = pd.DataFrame(errors.groupby(['GICS_SECTOR']).median())
grouped_mean_errors = pd.DataFrame(errors.groupby(['GICS_SECTOR']).mean())

grouped_median_errors2 = pd.DataFrame(errors.groupby(['VALUE_YEAR']).median())
grouped_mean_errors2 = pd.DataFrame(errors.groupby(['VALUE_YEAR']).mean())

# writing CSV
errors.to_csv('ind_errors'+data_id+'_'+parameters+'.csv',index=False)
